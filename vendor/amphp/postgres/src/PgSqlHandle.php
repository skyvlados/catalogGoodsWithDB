...
final class PgSqlHandle implements Handle
{
     ...
     * @param string $id DataBase identifier for determining which cached type table to use.
     */
    ...
    {
        if (!$this->isAlive()) {
            return new Success; // DataBase closed, no need to deallocate.
        }
    ...
    private function unlisten(string $channel): Promise
    {
        ...
        if (!$this->isAlive()) {
            $promise = new Success; // DataBase already closed.
        } else {
